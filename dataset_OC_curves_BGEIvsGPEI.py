...
if __name__ == '__main__':
    ...
    dataset_opt_direction_list = ['CurieTemperature_max','EnergyStorageDensity_max', 'DiffusionActivationEnergies_max','PerovskiteStability_max']
    initial_sample_percent_list = [0.05]
    root_path = 'RunData\\dataset_iteration_sample_OC_Curve_percent_BGSVR'
    repeat_num = 20
    ...
    for initial_sample_percent in initial_sample_percent_list:
        for dataset_opt_direction in dataset_opt_direction_list:
            ...
            dataset_path = root_path + '\\' + dataset_name + '_' + opt_direction+'_initial_sample_percent'+str(initial_sample_percent)+' '+str(initial_sample_num)
            ...
            for rn in range(repeat_num):
                ...
                for i in range(50):
                    ...
                    # BGEI
                    bg_ei_mean, bg_ei_std, bg_ei_train_r2, bg_ei_real_r2 = bagging_fit_predict(bg_ei_X_train, bg_ei_y_train, X_all,y_all, bg_ei_sample_index_list, 30, dataset_name, 'SVR') # 用GP作为基模型
                    bg_ei_recommend_index, bg_ei_value = ego_without_repeat(bg_ei_mean, bg_ei_std, bg_ei_cur_opt_value, opt_direction, bg_ei_sample_index_list) # 纯BG

                    # GPEI  无需交叉验证确定超参数
                    _index = [i for i in range(len(gp_ei_sample_index_list))]  # 全样本训练
                    gp_ei_mean, gp_ei_std,_index, _kernel, _r2_train = base_model_training(_index, gp_ei_X_train, gp_ei_y_train, X_all, gp_ei_sample_index_list, dataset_name, 'GP')
                    gp_ei_train_r2 = r2_score(y_all[gp_ei_sample_index_list], gp_ei_mean[gp_ei_sample_index_list])
                    gp_ei_real_r2 = r2_score(y_all, gp_ei_mean)
                    gp_ei_recommend_index, gp_ei_value = ego_without_repeat(gp_ei_mean, gp_ei_std, gp_ei_cur_opt_value, opt_direction, gp_ei_sample_index_list)
                    print(_kernel)
                    print('gp train: ',gp_ei_train_r2)
                    print('gp real: ',gp_ei_real_r2)
                    ...
                    gp_ei_sample_value_list.append(
                        [gp_ei_mean[gp_ei_recommend_index], gp_ei_std[gp_ei_recommend_index],
                         gp_ei_value[gp_ei_recommend_index]])

                    bg_ei_r2_list.append([bg_ei_real_r2, bg_ei_train_r2])
                    gp_ei_r2_list.append([gp_ei_real_r2, gp_ei_train_r2])
